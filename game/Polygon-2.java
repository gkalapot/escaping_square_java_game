...
//    for (Point p : shape) {
			...
			double x = ((p.x - center.x) * Math.cos(Math.toRadians(rotation)))
					- ((p.y - center.y) * Math.sin(Math.toRadians(rotation))) + 0 * center.x / 2 + position.x;
			double y = ((p.x - center.x) * Math.sin(Math.toRadians(rotation)))
					+ ((p.y - center.y) * Math.cos(Math.toRadians(rotation))) + 0 * center.y / 2 + position.y;
	...
	public boolean collides(Polygon other) {
		for (int i = 0; i < other.shape.length; i++) {
			if (this.contains(other.shape[i])) {
				return true;
			}
		}
		return false;
